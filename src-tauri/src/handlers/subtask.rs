...
#[tauri::command]
pub async fn get_subtasks(app: AppHandle, task_id: String) -> Result<serde_json::Value, String> {
    let conn = db::get_connection(&app).map_err(|e| e.to_string())?;
    ...
    let subtasks = stmt
        .query_map(rusqlite::params![&task_id], subtask_row_to_json)
        .map_err(|e| e.to_string())?
...
pub async fn update_subtask(
    ...
    if let Some(t) = title {
        conn.execute(
            "UPDATE subtasks SET title = ?, updatedAt = ? WHERE id = ?",
    ...
    if let Some(o) = order {
        conn.execute(
            "UPDATE subtasks SET \"order\" = ?, updatedAt = ? WHERE id = ?",
            rusqlite::params![o, &now, &id],
        )
        .map_err(|e| e.to_string())?;
    }
