...
pub fn get_db_path(_app: &AppHandle) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let data_dir = if cfg!(target_os = "macos") {
        PathBuf::from(std::env::var("HOME")?).join("Library/Application Support/deadline-tracker")
    } else if cfg!(target_os = "windows") {
        PathBuf::from(std::env::var("APPDATA")?).join("deadline-tracker")
    } else {
        // Linux
        PathBuf::from(std::env::var("HOME")?).join(".local/share/deadline-tracker")
    };
...
fn apply_migration_v1(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_tasks_isFocus ON tasks(isFocus) WHERE isFocus = TRUE;",
        [],
    )?;
    conn.execute(
